...
<div id="whole_page">
    ...
    <div id="option">
        ...
        <a href="TeacherSortServlet?sort=age" class="btn" style="background-color: yellow;float:left;margin-left: 15px;text-decoration:none">按年龄排序</a>
    </div>
    ...
    <div id="content" >
        <table id="table">
            ...
            <%
                if(!teachers.isEmpty())
                for(int i = 0; i < num_every_page; i++) {
                    int num =  i + (current_page - 1) * num_every_page;
            ...
            <%}%>
                ...
                <td class="cell"><%=teachers.get(num).getId()%></td>
                <td class="cell"><%=teachers.get(num).getName()%></td>
                <td class="cell"><%=teachers.get(num).getCollege()%></td>
                <td class="cell"><%=teachers.get(num).getMajor()%></td>
                <td class="cell"><%=teachers.get(num).getBirthday()%></td>
                <td class="cell"><%=teachers.get(num).getSalary()%></td>
                <td class="cell">
                    <a href="TeacherCheckServlet?id=<%=teachers.get(num).getId()%>" style="color:red">查看</a>
                    <a href="TeacherEditServlet?id=<%=teachers.get(num).getId()%>" style="color:red">修改</a>
                </td>
            </tr>
            <%} %>
            </tbody>
    ...
    <div id="bottom">
        <span id="data">第<%=current_page%>页,共<%=teachers.size()%>条,(每页显示<%=num_every_page%>条)</span>
...
<script>
    ...
    function del_confirm() {
        ...
        if (answer){
            ...
            let deletebox = [];
            let i = (current_page - 1) * num_every_page;
            <% if(!teachers.isEmpty())
                for(int i = (current_page - 1) * num_every_page; i < teachers.size() && i < (current_page)*num_every_page;i++){%>
                if(boxs[i - (current_page - 1) * num_every_page].checked)
